  ...
  catalogCache.pending = fs.readFile(path.join(__dirname, 'public', 'nfl_merged_players.json'), 'utf8')
    ...
    .then(async merged => {
      ...
      rows.forEach(row => {
        ...
        const player = { id: row.player_id, name: row.fullname, team, pos, ovr, maddenOvr, gameRating, passRating: number(row.pass_rating), rushRating: number(row.rush_rating), receiveRating: number(row.receive_rating), blockRating: number(row.block_rating), sleeperScore: context, performanceScore: row.performance?.score ?? null, value: Math.round((gameRating * .65 + context * .35) * 10) / 10, eligible: ELIGIBLE[pos], headshot: (row.headshot || '').replace('{formatInstructions}', 'w_96,c_fill') };
