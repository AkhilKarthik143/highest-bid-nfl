...
(() => {
  ...
  function renderDraft(game, room) {
    ...
    const rosterCard = (player, own) => `<div class="panel roster-card ${own ? 'my-roster' : ''}"><div class="roster-card-head"><strong>${own ? 'YOUR ROSTER' : esc(player.name)}</strong><span>${player.roster.length}/8</span></div><div class="roster-budget">${money(player.budget)} remaining</div><div class="roster-players">${player.roster.map(item => `<span class="roster-chip"><b>${esc(item.pos)}</b> ${esc(item.name)}</span>`).join('') || '<span class="roster-empty">No players drafted yet</span>'}</div></div>`;
