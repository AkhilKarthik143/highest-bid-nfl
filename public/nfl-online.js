...
(() => {
  ...
  function renderDraft(game, room) {
    const draft = room.draft, team = draft.currentTeam || {}, mine = me(game), leader = room.players.find(player => player.id === draft.highestBidder);
    const folded = draft.folded.includes(game.onlineSocket?.id); const legalMax = mine ? Math.max(0, mine.budget - (slots.length - mine.roster.length - 1)) : 0;
