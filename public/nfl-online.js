...
(() => {
  ...
  function renderDraft(game, room) {
    ...
    const needs = player.roster.map(item => item.pos); const has = position => needs.includes(position); const needText = ['QB','RB','WR','TE'].filter(position => !has(position)).join(', ') || 'FLEX';
    const rosterCard = (player, own) => { const open = Math.max(0, slots.length - player.roster.length); const max = Math.max(0, player.budget - Math.max(0, open - 1)); return `<div class="panel roster-card ${own ? 'my-roster' : ''}"><div class="roster-card-head"><strong>${own ? 'YOUR TEAM' : esc(player.name)}</strong><span>${player.roster.length}/8</span></div><div class="roster-budget">${money(player.budget)} budget · ${open} open slots</div>${own ? `<div class="roster-summary"><span>MAX BID <b>${money(max)}</b></span><span>NEEDS <b>${esc(needText)}</b></span></div>` : ''}<div class="roster-players">${player.roster.map(item => `<span class="roster-chip"><b>${esc(item.pos)}</b> ${esc(item.name)} <em>${money(item.price)}</em></span>`).join('') || '<span class="roster-empty">No players drafted yet</span>'}</div></div>`; };
  ...
  renderDraft = function (game, room) {
